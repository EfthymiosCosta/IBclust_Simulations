...
# Mildly-separated non-spherical (elliptical) clusters
clusters <- 4
overlap <- 0.1
rows <- 500
columns <- 8
...
cat_ratio <-  0.5
nreps <- 100
res_df_knee_nonsep <- data.frame(mi2 = numeric(),
                                 mi3 = numeric(),
                                 mi4 = numeric(),
                                 mi5 = numeric(),
                                 mi6 = numeric(),
...
for (i in 1:nreps){
  set.seed(i)
  # Construct artificial data set - sphericity set to FALSE
  mixsimaux <- MixSim(BarOmega = overlap, PiLow=pi_val,
                      K = clusters, p = columns, resN = 1000000, sph = FALSE)
  ...
  outdibmix2 <- IBclust::DIBmix(X = mixdt1df,
                                ...
                                nstart = 100,
                                verbose = FALSE)
  res_df_knee_nonsep[i, 1] <- outdibmix2$MutualInfo
  outdibmix3 <- IBclust::DIBmix(X = mixdt1df,
                                ...
                                nstart = 100,
                                verbose = FALSE)
  res_df_knee_nonsep[i, 2] <- outdibmix3$MutualInfo
  outdibmix4 <- IBclust::DIBmix(X = mixdt1df,
                                ...
                                nstart = 100,
                                verbose = FALSE)
  res_df_knee_nonsep[i, 3] <- outdibmix4$MutualInfo
  outdibmix5 <- IBclust::DIBmix(X = mixdt1df,
                                ...
                                nstart = 100,
                                verbose = FALSE)
  res_df_knee_nonsep[i, 4] <- outdibmix5$MutualInfo
  outdibmix6 <- IBclust::DIBmix(X = mixdt1df,
                                ...
                                nstart = 100,
                                verbose = FALSE)
  res_df_knee_nonsep[i, 5] <- outdibmix6$MutualInfo
  outdibmix7 <- IBclust::DIBmix(X = mixdt1df,
                                ...
                                nstart = 100,
                                verbose = FALSE)
  res_df_knee_nonsep[i, 6] <- outdibmix7$MutualInfo
  outdibmix8 <- IBclust::DIBmix(X = mixdt1df,
                                ...
                                nstart = 100,
                                verbose = FALSE)
  res_df_knee_nonsep[i, 7] <- outdibmix8$MutualInfo
