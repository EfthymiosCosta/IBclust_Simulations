...
for (i in 1:nreps) {
  ...
  # Generate artificial dataset (sph = FALSE)
  ...
  for (C_val in C_min:C_max) {
    ...
    # Store results
    res_df_gap_nonsep <- rbind(res_df_gap_nonsep, data.frame(
      rep = i,
      C = C_val,
  ...
  saveRDS(res_df_gap_nonsep, file = 'res_df_gap_nonsep.RDS')
}
